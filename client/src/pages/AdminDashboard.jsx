  ...
  return (
    <Layout title="HR Admin Dashboard">
      <div className="mb-6">
        ...
        <p className="mt-1 text-slate-500">Key metrics and today's attendance tracking.</p>
      </div>
        ...
        <>
          ...
          <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-100">
              ...
              <p className="mt-2 text-3xl font-bold text-slate-800">{totalEmployees}</p>
            </div>
            <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-100">
              <p className="text-sm font-medium text-slate-500">Today's Attendance Rate</p>
              <div className="mt-2 flex items-baseline gap-2">
                <p className="text-3xl font-bold text-slate-800">{attendanceRate}%</p>
                <p className="text-sm text-slate-500">({presentToday} present)</p>
              </div>
            </div>
            <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-100">
              <p className="text-sm font-medium text-slate-500">Pending Leave Requests</p>
              <p className="mt-2 text-3xl font-bold text-slate-800">{pendingLeavesCount}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="overflow-hidden rounded-xl bg-white shadow-sm border border-slate-100">
              <div className="border-b border-slate-200 px-6 py-4">
                <h3 className="font-semibold text-slate-800">Department Headcount</h3>
              </div>
              <div className="p-0">
                {deptList.length === 0 ? (
                  <p className="p-6 text-sm text-slate-500">No departments found.</p>
                ) : (
                  <ul className="divide-y divide-slate-100">
                    {deptList.map(([dept, count]) => (
                      <li key={dept} className="flex justify-between items-center px-6 py-4 hover:bg-slate-50/50">
